...
export default class WaitingList extends Component {
  ...
  runSignalR(merchantInfo, getWaitingList) {
  ...
  renderServiceModal(services) {
    if(services.length > 0){
      return (
        <React.Fragment>
          <Text style={{ color: '#1266AE', fontWeight: '700', fontSize: hp("2%"), }}>
            Service
        </Text>
  
          <View style={{ backgroundColor: '#F1F7FB', display: 'flex', flexDirection: 'row', width: '100%', padding: 15, marginTop: 10 }}>
            <Text style={{ width: '50%', color: '#333', fontWeight: '500', fontSize: hp('1.6%') }}>Name</Text>
            <Text style={{ width: '35%', color: '#333', fontWeight: '500', fontSize: hp('1.6%') }}>Duration</Text>
            <Text style={{ width: '15%', color: '#333', fontWeight: '500', fontSize: hp('1.6%') }}>Price</Text>
          </View>
          {services.map((sv, index) => {
            return (
              <View key={index} style={{ display: 'flex', flexDirection: 'row', width: '100%', padding: 15 }}>
                <Text style={{ width: '50%', color: '#333', fontSize: hp('1.6%') }}>{sv.serviceName}</Text>
                <Text style={{ width: '35%', color: '#333', fontSize: hp('1.6%') }}>{sv.duration} min</Text>
                <Text style={{ width: '15%', color: '#333', fontSize: hp('1.6%') }}>$ {parseFloat(sv.price).toFixed(2)}</Text>
              </View>
            )
          })}
        </React.Fragment>
    ...
    if(products.length > 0){
      return (
        <React.Fragment>
          ...
          <View style={{ backgroundColor: '#F1F7FB', display: 'flex', flexDirection: 'row', width: '100%', padding: 15, marginTop: 10 }}>
            <Text style={{ width: '50%', color: '#333', fontWeight: '500', fontSize: hp('1.6%') }}>Name</Text>
            <Text style={{ width: '35%', color: '#333', fontWeight: '500', fontSize: hp('1.6%') }}>Amount</Text>
            <Text style={{ width: '15%', color: '#333', fontWeight: '500', fontSize: hp('1.6%') }}>Price</Text>
          </View>
          {products.map((product, index) => {
            return (
              <View key={index} style={{ display: 'flex', flexDirection: 'row', width: '100%', padding: 15 }}>
                <Text style={{ width: '50%', color: '#333', fontSize: hp('1.6%') }}>{product.productName}</Text>
                <Text style={{ width: '35%', color: '#333', fontSize: hp('1.6%') }}>{product.quantity}</Text>
                <Text style={{ width: '15%', color: '#333', fontSize: hp('1.6%') }}>$ {parseFloat(product.price).toFixed(2)}</Text>
              </View>
            );
          })}
        </React.Fragment>
      ...
      return (
        <React.Fragment>
          ...
          {extras.map((extra, index) => {
            return (
              <View key={index} style={{ display: 'flex', flexDirection: 'row', width: '100%', padding: 15 }}>
                <Text style={{ width: '50%', color: '#333', fontSize: hp('1.6%') }}>{extra.extraName}</Text>
                <Text style={{ width: '35%', color: '#333', fontSize: hp('1.6%') }}>{extra.duration} min</Text>
                <Text style={{ width: '15%', color: '#333', fontSize: hp('1.6%') }}>$ {parseFloat(extra.price).toFixed(2)}</Text>
              </View>
            );
          })
          }
  ...
  renderTotalModal(appointmentDetail) {
    if(appointmentDetail){
      ...
      for(let i = 0; i<extras.length;i++){
        totalDuration+=extras[i].duration;
        totalPrice += extras[i].price;
      }
      ...
      return (
        <View style={{
          backgroundColor: '#F1F7FB', display: 'flex', flexDirection: 'row',
          width: '100%', padding: 15, marginTop: 40
        }}>
          <View style={{ display: 'flex', flexDirection: 'row', width: '50%', justifyContent: 'space-between', paddingRight: 50 }}>
          ...
          <View style={{ display: 'flex', flexDirection: 'row', width: '50%', justifyContent: 'space-between', paddingLeft: 50 }}>
            <Text style={{ color: '#1266AF', fontWeight: '500', fontSize: hp('1.6%') }}>
              Total price :
            </Text>
            <Text style={{ color: '#1266AF', fontWeight: '700', fontSize: hp('1.6%') }}>
    ...
    if(appointmentDetail){
      return (
        <View style={{ display: 'flex', flexDirection: 'row', marginTop: 30 }}>
          <Text style={{ color: '#333', fontSize: hp('1.6'), fontWeight: '600' }}>Estimate waiting time : </Text>
          <Text style={{ color: '#333', fontSize: hp('1.6'), fontWeight: '600' }}>{convertMinsToHrsMins(appointmentDetail.waitingTime)}</Text>
        </View>
      )
    } 
  }
  ...
  renderDetailAppointment() {
    ...
    const { userFullName, phoneNumber, id, products, services, extras,waitingTime } = appointmentDetail;
       ...
       isVisible={this.state.isModal}>
        <View style={{ alignSelf: 'center', backgroundColor: '#ffffff', height: '90%', width: '90%' }}>
          <View style={{
            ...
            flexDirection: 'row', backgroundColor: '#1266AF', justifyContent: 'center', position: 'relative'
          }}>
            <Text style={{ color: '#ffffff', fontWeight: '700', fontSize: hp('2.5%') }}>Appointment Detail</Text>
            <TouchableOpacity
          ...
          <View style={{ flex: 1, padding: 20 }}>
            <View style={{ display: 'flex', flexDirection: 'row', }}>
              <Text style={{ width: '40%', fontSize: hp("1.8%"), color: '#333', fontWeight: '500' }}>ID</Text>
              <Text style={{ fontSize: hp("1.8%"), color: '#333', fontWeight: '500' }}>#{id}</Text>
            </View>
            <View style={{ display: 'flex', flexDirection: 'row', marginTop: 10 }}>
              <Text style={{ width: '40%', fontSize: hp("1.8%"), color: '#333', fontWeight: '500' }}>Customer</Text>
              <Text style={{ fontSize: hp("1.8%"), color: '#333', fontWeight: '500' }}>{userFullName}</Text>
            </View>
            <View style={{ display: 'flex', flexDirection: 'row', marginTop: 10 }}>
              <Text style={{ width: '40%', fontSize: hp("1.8%"), color: '#333', fontWeight: '500' }}>Phone number</Text>
              <Text style={{ fontSize: hp("1.8%"), color: '#333', fontWeight: '500' }}>{phoneNumber}</Text>
            </View>
            <View style={{ marginTop: 10, borderWidth: 0.5, borderColor: "#dddddd" }}></View>
          </View>
      ...
      return (
        <Item onPress={() => this.openModal(item)} key={index}>
          ...
          <TimeWrapper>
            ...
            <Text style={styles.itemWaitingTime}>
              {convertMinsToHrsMins(item.waitingTime)}
        </Text>
          </TimeWrapper>
  ...
  renderNoItem() {
    return (
      <NoItem>
        <Text style={{ color: '#DADCDE', fontSize: hp("4%"), fontWeight: '600' }}>
          +
        </Text>
      </NoItem>
    )
        ...
        <SignInLeft>
          ...
          <SignInLeft.Item>
            <Image
              ...
              style={{ marginTop: -5, width: 20, resizeMode: "contain" }}
            />
            <Text style={styles.txtEstimste}>
              Estimate waiting time
              </Text>
            <Text style={styles.txtWaitingTime}>
              {convertMinsToHrsMins(this.getTotalWaitingTime())}
              </Text>
          </SignInLeft.Item>
        </SignInLeft>
